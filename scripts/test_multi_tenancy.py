...
def login(email, password):
    ...
    resp = httpx.post(f"{API_URL}/auth/login", data={
        "username": email,
        "password": password,
    })
    resp.raise_for_status()
...
def test_multi_tenancy():
    ...
    # Ingest dummy repo
    ingest_resp = httpx.post(f"{API_URL}/ingest/", headers=headers_a, json={
        "repo_path": r"d:\AIEKP\dummy_repo"
    }, timeout=30)
    ...
    search_a = httpx.post(f"{API_URL}/search/", headers=headers_a, json={"query": "vault code"})
    ...
    search_b = httpx.post(f"{API_URL}/search/", headers=headers_b, json={"query": "vault code"})
