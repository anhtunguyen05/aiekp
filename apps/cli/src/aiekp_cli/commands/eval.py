...
def get_db_path():
    ...
    default_db_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../../api/telemetry.db"))
    db_path = os.environ.get("TELEMETRY_DB_PATH", default_db_path)
    if not os.path.exists(db_path):
        console.print(f"[bold yellow]Warning:[/bold yellow] telemetry.db not found at {db_path}.")
    return db_path

@eval_app.command("run")
def run_eval():
    """Run RAGAS evaluation on recent traces."""
    console.print("[bold blue]Running RAGAS evaluation on telemetry data...[/bold blue]")
    try:
        from ragas import evaluate
        from ragas.metrics import answer_relevancy, faithfulness
        from datasets import Dataset
    except ImportError:
        console.print("[bold red]ragas or datasets library not found. Please install them using `pip install ragas datasets`.[/bold red]")
        raise typer.Exit(1)
    ...
    try:
        ...
        for row in rows:
            ...
            data["contexts"].append([s.get("label", "") + " " + s.get("snippet", "") for s in sources])
...
def export_jsonl(output: str = "finetune_data.jsonl"):
    ...
    try:
        ...
        with open(output, 'w', encoding='utf-8') as f:
            for row in rows:
                ...
                if not answer:
                    ...
                    cursor.execute("SELECT outputs FROM trace_spans WHERE trace_id = ? AND name = 'process_query'", (row["trace_id"],))
        ...
        console.print(f"[bold green]Successfully exported {count} records to {output}[/bold green]")
