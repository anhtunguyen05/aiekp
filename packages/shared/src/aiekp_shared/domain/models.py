...
class Repository(BaseModel):
    ...
    uri: str = Field(
        ..., description="URI of the repository (e.g., local path or git URL)"
    )
    branch: Optional[str] = Field(None, description="Branch name if applicable")
    commit_hash: Optional[str] = Field(
        None, description="Commit hash for the specific state"
    )
    language_stack: List[str] = Field(
        default_factory=list, description="List of primary programming languages"
    )
...
class ASTNode(BaseModel):
    ...
    type: str = Field(
        ..., description="Type of the node (e.g., Function, Class, Import)"
    )
    start_line: int = Field(
        ..., description="Start line of the node in the source file"
    )
    ...
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Extracted structural info above AST"
    )
...
class KnowledgeNode(BaseModel):
    ...
    label: str = Field(
        ..., description="Graph node label (e.g., File, Class, Method, ADR)"
    )
