...
class ContextService(IContextService):
    ...
    def __init__(
        self, intent_analyzer: IIntentAnalyzer, knowledge_client: IKnowledgeEngineClient
    ):
    ...
    async def retrieve_context(self, query: str) -> EvidencePayload:
        ...
        search_query = (
            " ".join(intent.semantic_keywords)
            if intent.semantic_keywords
            else intent.query
        )
        search_results = await self.knowledge_client.search_nodes(
        ...
        return EvidencePayload(
            original_query=query, intent=intent, nodes=nodes, summary=summary
        )
