...
class AiekpAstNode(BaseModel):
    ...
    type: str = Field(
        ...,
        description="The type of the node (e.g., 'class_definition', 'function_definition')",
    )
    ...
    start_point: tuple[int, int] = Field(
        ..., description="Start point as (row, column)"
    )
    end_point: tuple[int, int] = Field(..., description="End point as (row, column)")
    children: List["AiekpAstNode"] = Field(
        default_factory=list, description="Child nodes"
    )
    properties: Dict[str, Any] = Field(
        default_factory=dict, description="Additional language-specific properties"
    )
