...
class ReasoningOrchestrator:
    ...
    async def fetch_context_node(self, state: GraphState):
        ...
        try:
            ...
            return {"error": str(e), "next_agent": "FINISH"}
    ...
    async def supervisor_node(self, state: GraphState):
        ...
        history_str = "\\n".join(
            [f"{type(m).__name__}: {m.content}" for m in state.get("messages", [])]
        )
        ...
        """
        response_str = await self.llm_generator.generate(
            prompt, "You are a JSON-only supervisor agent."
        )
        ...
        try:
            ...
            # Fallback
            new_msg = AIMessage(
                content=f"Error parsing supervisor output: {str(e)}\nRaw LLM output was:\n{response_str}",
                name="Supervisor",
            )
            return {
                "next_agent": "FINISH",
    ...
    async def _specialized_agent_node(
        self, state: GraphState, role_name: str, system_prompt: str
    ):
    ...
    async def architect_node(self, state: GraphState):
        return await self._specialized_agent_node(
            state,
            "Architect",
            "You are a Software Architect AI. Focus on system design, SOLID principles, dependencies, and structural integrity.",
        )
    ...
    async def qa_node(self, state: GraphState):
        return await self._specialized_agent_node(
            state,
            "QA",
            "You are a QA Engineer AI. Focus on edge cases, testability, logic bugs, and potential failures.",
        )
    ...
    async def security_node(self, state: GraphState):
        return await self._specialized_agent_node(
            state,
            "Security",
            "You are a Security Expert AI. Focus on identifying vulnerabilities (XSS, SQLi, insecure data, etc.)",
        )
